...
* **实现步骤** :
  ...
  1. 去除重复的充电站（基于 `City` 和 `Station` 列），只保留每个站第一次出现的记录。
  1. 对这些不重复的站，按 `City` 和 `cpo_name_pinyin` 进行分组 (`groupby`)。
  1. 在每个组内，使用累积计数 (`cumcount()`) 生成一个从 0 开始的排名，然后加 1。
  1. 将这个数字格式化为**三位、前面补零**的字符串 (e.g., `1` -> `001`)。
